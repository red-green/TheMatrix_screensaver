...
void draw() {
  ...
  if (a > videoScale) {
    ...
    for (int i = 0; i < cols; i++) {
      ...
      if (buffer[i][rows - 2] == ' ') {
        zoom[i][rows-1] = random(-125,325);
        colors[i][rows-1] = color(red(colors[i][rows-1]),green(colors[i][rows-1]),blue(colors[i][rows-1]),random(50,200));
      } else {
        zoom[i][rows-1] = zoom[i][rows-2];
        colors[i][rows-1] = color(red(colors[i][rows-1]),green(colors[i][rows-1]),blue(colors[i][rows-1]),alpha(colors[i][rows-2]));
      }
    }
